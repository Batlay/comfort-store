...
function CartPage() {
  ...
  return ( 
    <section className="py-20">
      {cart.length === 0 ? <h2 className="text-xl">Your cart is currently empty</h2> : <h2 className="capitalize font-medium text-xl tracking-wider">shopping cart</h2>}
      <div className="divider"></div>
      {cart.length > 0 && 
      <div className="grid grid-cols-12 pt-4 gap-8">
        <div className="lg:col-span-8">
          {cart.map(product => {
            const {title, company, image, price, color} = product
            ...
            <article className="flex flex-row">
              ...
              <div className="ml-16 w-48">
                ...
                <p className="capitalize text-sm mt-4 flex items-center gap-x-2 ">color: 
                  <span className="badge badge-xs rounded-full" style={{backgroundColor: color}}></span>
                </p>
              </div>
        ...
        <div className="lg:col-span-4">
          ...
          <button className="btn btn-primary w-full uppercase rounded-lg mt-5">
            please login
          </button>
        </div>
      </div>}
